...
const char API_VERSION[8] = "1.2.2.0";
...
FX3_FWDWNLOAD_ERROR_CODE CCyFX3Device::DownloadFw(char* data, int datalen, FX3_FWDWNLOAD_MEDIA_TYPE enMediaType)
{
	FX3_FWDWNLOAD_ERROR_CODE ErroCode = SUCCESS;
	// call api to download the image
	if (enMediaType == RAM)
		ErroCode = DownloadFwToRam((PUCHAR)data, datalen, 0xA0);
	else if (enMediaType == I2CE2PROM)        
		ErroCode = DownloadUserIMGtoI2CE2PROM((PUCHAR)data, datalen, 0xBA);
	else if (enMediaType == SPIFLASH)
		ErroCode = DownloadUserIMGtoSPIFLASH((PUCHAR)data, datalen, 0xC4);
	else
		ErroCode = INVALID_MEDIA_TYPE;
	return ErroCode;
}
