...
int cyusb_open(void)
{
	...
	//uint32_t max_dev_cnt = 2;
	//uint32_t vid_list[] = {0x04B4, 0x04B4};
	//uint32_t pid_list[] = {0x00F2, 0x00F3};
	//char* desc_list[] = {"NLPR - sir1b", "Cypress Bootloader Device"};
	char* desc = "NLPR Sir Module";

	maxdevices = 0;

	for(r = 0; r < g_nCustomFx3DevIDCnt; r++)
	{
		vpd[maxdevices].vid = g_nCustomFx3VenID[r];
		vpd[maxdevices].pid = g_nCustomFx3DevID[r];
		strncpy(vpd[maxdevices].desc, desc, strlen(desc));
		vpd[maxdevices].desc[strlen(desc)] = '\0';   /* Make sure of NULL-termination. */
		++maxdevices;
	}

	//printf("Begin initialize libusb.\n");
	r = libusb_init(NULL);
	if (r) {
